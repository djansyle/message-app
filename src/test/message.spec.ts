...
import { chance, authenticateUser, createChannel, addChannelMember, sendMessage, listMessages } from './helper';
...
describe('Message', function() {
  ...
  describe('Query#message', function () {
    it('SHOULD be able to retrieve messages in descending order', async function() {
      ...
      const { response, error } = await listMessages({ accessToken, channelId });
      expect(error).toBeNull();

      const receivedMessages = response.data.messages.edges.map(R.path(['node', 'text']));
      expect(response.data.messages.edges).toMatchObject(expect.arrayContaining([
    ...
    it('SHOULD be able to receive messages WHEN user is a member of the channel', async function() {
      ...
      await addChannelMember({ accessToken, channelId, userId: member.id });
      await Promise.all(R.times(() => sendMessage({ accessToken, channelId, text: chance.sentence() }), 10));

      const { response, error } = await listMessages({ accessToken: member.accessToken, channelId });
      expect(error).toBeNull();
    ...
    it('SHOULD be able to retrieve messages GIVEN first and after parameter', async function() {
      ...
      let { response, error } = await listMessages({ accessToken, channelId, first: 2 });
      expect(error).toBeNull();
      expect(response.data.messages.edges.length).toBe(2);

      ({ response, error } = await listMessages({ accessToken, channelId, first: 20, after: response.data.messages.edges[1].cursor }));
      expect(error).toBeNull();
    ...
    it('SHOULD return a NOT_CHANNEL_MEMBER error WHEN the sender is not a member', async function() {
      ...
      const { response, error } = await listMessages({ accessToken: accessToken2, channelId });
      expect(response).toBeUndefined();
      expect(error).toBeDefined();
      expect(R.path(['graphQLErrors', 0, 'code'], error)).toBe('NOT_CHANNEL_MEMBER');
    });
    ...
    it('SHOULD return a CHANNEL_NOT_FOUND error WHEN channel does not exists', async function() {
      ...
      expect(response).toBeUndefined();
      expect(error).toBeDefined();
      expect(R.path(['graphQLErrors', 0, 'code'], error)).toBe('CHANNEL_NOT_FOUND');
    });
  });
});